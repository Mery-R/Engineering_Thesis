...
// Maximum timestamp (ms) considered valid - larger values are ignored
const uint64_t TimeManager::MAX_VALID_UNIX_MS = 2147483647000ULL;
...
void IRAM_ATTR TimeManager::handlePPS() {
    ...
    if (timeValid && baseUnixMs >= MIN_VALID_UNIX_MS && baseUnixMs <= MAX_VALID_UNIX_MS) {
...
void TimeManager::updateFromGps(uint64_t gpsUnixMs) {
    ...
    if (gpsUnixMs < MIN_VALID_UNIX_MS || gpsUnixMs > MAX_VALID_UNIX_MS) {
        Serial.println("[TimeManager] updateFromGps: gps time out of valid range, ignoring");
        return;
    }
...
void TimeManager::syncTime(uint64_t unixMs) {
    if (unixMs >= MIN_VALID_UNIX_MS && unixMs <= MAX_VALID_UNIX_MS) {
        ...
        Serial.println("[TimeManager] syncTime: rejected invalid unixMs");
    }
}
...
bool TimeManager::isSynchronized() {
    if (timeValid && baseUnixMs >= MIN_VALID_UNIX_MS && baseUnixMs <= MAX_VALID_UNIX_MS) return true;
    return false;
}
